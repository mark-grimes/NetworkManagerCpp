...
SCENARIO( "Test that version gives the correct git information", "[version]" )
{
	...
	{
		CHECK( libnm::version::GitDescribe != nullptr );
		CHECK( std::char_traits<char>::length(libnm::version::GitDescribe) > 0 ); // Not sure how to check this consistently
		CHECK( libnm::version::GitHash[41] == 0 );
		CHECK( std::char_traits<char>::length(libnm::version::GitHash) == 40 ); // Check there are no nulls half way through
	}
	WHEN( "Checking the git hash is all valid hex characters" )
	{
		...
		{
			if( !isHexChar(libnm::version::GitHash[index]) ) break;
		}
		INFO( std::string("The git hash '")+libnm::version::GitHash+"' is not valid hex characters" );
